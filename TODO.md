...
- [x] **Zona condivisione con OpenGraph/meta/SEO per ROTTE e ITINERARI** (11/07) — FATTO nella Zona Media: `poi.php` e `route.php` generano le landing OG (titolo/descrizione/immagine dai template `og_templates` configurabili nell'admin, scheda OpenGraph); `trip.php` già esisteva per gli itinerari; deep-link tracciati via `go.php`. Resta da agganciare i bottoni "Condividi" della webapp a queste landing (oggi usano ancora `?poi=`/`?route=` diretti).
