...
- [ ] **Pulizia lasciata dalla revisione del 19/08** (non bloccante, verificata):
  - `renderPubPOIBoard` mette l'intero POI in JSON dentro `onclick`: passare all'indice `_poiTabIndex` come nelle altre liste.
  - `window._savedViewRestored` viene riusato con due significati diversi: separare i due segni.
  - `window._zTop`/`_zModalTop` crescono e basta: valutare un azzeramento quando non c'e' piu' nessuna finestra aperta.
