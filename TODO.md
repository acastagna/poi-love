...
> **Dove si guarda lo stato dei lavori, in ordine di utilita':**
> 1. **https://sal.poilove.com/avanzamento.html** — dove siamo adesso. Si aggiorna da sola, legge il programma vero dal database (tabella `programma`, migrazione 093). E' il posto da guardare tutti i giorni.
> 2. **https://sal.poilove.com/progetto.html** — il progetto per intero: architettura, disegni approvati, calendario, rischi. Sorgente `docs/PROGETTO-GLOBALE.html`.
> 3. Questo file e `PROGETTO-PROFILI-E-SCHEDE.md` — il dettaglio scritto, per chi deve costruire.
>
> Quando un blocco cambia stato si scrive nel database, non nei file: la pagina si aggiorna da sola.

- [x] **Canale realtime rotto dal 18/08** — RISOLTO il 22/08 (v5.63, mig 169) con la coppia decisa con Alessandro: push web vere spedite dalla macchina ad app chiusa (postino ogni minuto, levetta browser gia' esistente) + ricontrollo periodico ogni 30 secondi a pagina aperta. Il realtime resta agganciato nel codice: se un giorno si montera' il servizio, suonera' di nuovo da solo. Resta da fare la prova su dispositivo vero con Alessandro (il browser di lavoro nega il permesso d'ufficio).
- [ ] **Realtime self-hosted (facoltativo, dopo il lancio)**: con push + ricontrollo copre poco in piu'; si valuta solo se l'istantaneo a pagina aperta diventera' importante (presenza compagnie, posizione condivisa).
- [ ] **Quando si accende la quarta lingua, le categorie devono leggerla**: l'infrastruttura c'e' (categorie_per_lingua sul database, mig 167, provata), ma l'app legge ancora le colonne label_it/sq/en. Il giorno che una lingua oltre le tre si accende nel registro, il caricamento categorie va spostato sulla funzione. (Rilievo della revisione 22/08.)
- [ ] **Levette per gli avvisi "di conto"**: promemoria abbonamento, avvisi di livello e invito a ricambiare oggi non hanno una riga propria nel pannello notifiche; il postino li manda a chi ha il canale browser acceso su qualunque tipo (regola documentata nel postino). Quando si rivedra' il pannello, dare loro una voce propria.
- [x] **Mac e iPhone: avvisi che non comparivano** — RISOLTO il 22/08, tutto verificato dal vivo su entrambi. Cause trovate una per una: Full immersion "Lavoro" attiva (sincronizzata tra i dispositivi via iCloud), notifiche di Chrome spente a livello macOS, interruttore "notifiche durante condivisione schermo" spento con Snagit attivo, ricevitore vecchio in cache sull'iPhone, e per ultima la voce "Google Chrome Helper (Alerts)" spenta in macOS (e' il componente che consegna le notifiche dei siti: la voce grande di Chrome accesa non basta). L'impianto push era sano; ora ha in piu' il rinnovo automatico per generazione (PUSH_GEN), l'avviso sempre mostrato sui dispositivi Apple (regola di iOS sulle push zittite) e la spia diagnostica sw-diario.
- [ ] **Spegnere la spia sw-diario** quando le push saranno assestate da qualche settimana: togliere _spia() da webapp/sw.js e il file sw-diario.php dalla macchina (nessun dato personale, solo fasi e versioni, ma a regime non serve).
- [x] **Cartografia** - RISOLTA il 27/08: chiave gratuita Carto richiesta da Alessandro, arrivata e montata; il vestito Voyager e' tornato su tutti e cinque i punti (stradale, nomi sul satellite, lente, minimappa, itinerari), con ripiego AUTOMATICO su OSM dopo quattro tegole rotte: mai piu' timbri davanti alla gente. Chiave in • chiavi/carto.txt, 5 milioni di tegole/mese, attribuzione CARTO+OSM visibile (obbligo del gratis). Verificato dal vivo, v5.77.
- [ ] **Il raster di Carto andra' in pensione**: la mail della chiave lo dice chiaro ("raster is being retired") e spinge al vettoriale. Prima o poi la mappa va portata su tegole vettoriali (MapLibre GL): piu' nitide, restilizzabili, e la stessa strada vale per l'app nativa. Da pianificare con calma, non urgente.
...
- [ ] **La chiave di Google per le audioguide** (lo fa Alessandro): si prende su https://aistudio.google.com/apikey e si carica nel segreto `GOOGLE_TTS_KEY` delle funzioni. E' l'unica cosa che manca: tutto il resto dello studio di doppiaggio e' costruito, pubblicato e provato fino a Google.
- [ ] **Decidere se accendere l'intelligenza sulla nostra macchina**: c'e' ed e' gratis (sedici parole al secondo, misurate), ma risponde peggio e in albanese sbaglia. Interruttore in Connessioni AI. Da riprovare con un modello piu' grande quando serve.
- [ ] **Indice per la ricerca nella conoscenza**: oggi la ricerca scorre tutti i pezzi, che sotto qualche migliaio e' piu' veloce di un indice. Quando i documenti caricati crescono, va messo un indice ivfflat su `conoscenza_pezzi.vettore`.
- [ ] **Le promesse dei livelli vanno mantenute a mano**: sei vantaggi del catalogo (assistenza diretta, prima fila sulle novita', la dedica sulla rotta, il nome su un itinerario, i quindici luoghi in omaggio, il QR della vetrina) non li fa rispettare nessun programma. O si legano a una regola vera, o va ricordato che sono impegni nostri.

- [ ] **Le domande della pagina documenti riservati hanno bisogno di qualcuno in ascolto**: le risposte arrivano quando una sessione gira sul Mac e lancia `scripts/coda.sh prendi`. Se una domanda resta in attesa a lungo, aprire una sessione e svuotare la coda.
