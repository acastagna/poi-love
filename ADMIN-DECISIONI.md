...
- 🔨 A2 Scheda Utente riusabile: **MVP FATTO e LIVE.** `admin/js/user-card.js` (drawer laterale: avatar, stato, livello, punti, data, motivo + azioni Messaggia/Sospendi/Banna/Riattiva che riusano le funzioni del pannello). **I profili ora si cliccano** in Utenti e in Dashboard (ultimi iscritti) → apre la scheda. Verificato dal vivo con harness. Da espandere (orchestrazione A2): contenuti creati dall'utente, segnalazioni, conversazioni supporto, IP registrazione/ultimo accesso.
